...
def main():
	"""
	Writes out "transcript-userId.json" with the history of the user
	...
	transcriptFileName = 'transcript-{0}.json'.format(user_id)
	transcript = loadTranscript(transcriptFileName)
	if args.resumePrevious or args.resumeNext:
		tempFileName = getTempFileName(user_id)
	...
	transcript = populateTranscript(user_id, accessToken, transcript, beforeId, stopId, pages)
...
def populateTranscript(user_id, accessToken, transcript, beforeId, stopId, pageLimit=None):
	complete = False
	pageCount = 0
	endpoint = 'https://api.groupme.com/v3/direct_messages'
	headers = {
		...
		#'Referer': 'https://web.groupme.com/direct_messages/',
	...
	with open(tempFileName, 'wb') as tempFile:
		while not complete:
			...
			if beforeId is not None:
				params = {'other_user_id': user_id, 'before_id': beforeId}
			else:
...
def getTempFileName(user_id):
	return 'temp-transcript-{0}.json'.format(user_id)
